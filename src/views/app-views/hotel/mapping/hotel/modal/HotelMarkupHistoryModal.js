...
const HotelMarkupHistoryModal = (props) => {
    ...
    const findLog = async (record) => {
        ...
        const response = await LogService.findMarkupHotelByHotelId({
            sourceSystem: channel,
            hotelId: record.id,
        })

        if (response.success) {
            setData(response.logMarkupChanges)
        }
    }
            ...
            >
                ...
                >
                    <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                        <HotelMarkupHistoryTable data={data} />
                    </Col>
                </Spin>
            </StyleModal>
        </>
...
export default HotelMarkupHistoryModal
